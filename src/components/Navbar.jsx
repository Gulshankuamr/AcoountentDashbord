...
import {
  ...
  Settings, HelpCircle, User, Shield, Inbox, X, Calculator,
} from 'lucide-react'
...
const NotifDropdown = ({ onClose, basePath }) => {
  ...
  const handleItemClick = async (n) => {
    ...
    onClose()
    navigate(`${basePath}/my-notifications`)
  }
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 border-t border-gray-100 bg-gray-50">
        <button
          onClick={() => { onClose(); navigate(`${basePath}/my-notifications`) }}
...
const Navbar = ({ onMenuClick, isCollapsed }) => {
  ...
  // base path for all navigation links
  ...
  const handleLogout = () => {
    ...
    logout()
    navigate('/login')   // both roles → accountant login page
  }

  const initials       = getInitials(user?.name)
  const avatarGradient = isAccountant
    ? 'from-emerald-500 to-teal-600'
    : getAvatarGradient(user?.name)

  // Title shown in top-left
  const dashboardTitle = isAccountant ? 'Accountant Dashboard' : 'Admin Dashboard'
  const dashboardSub   = isAccountant ? 'Manage fees and school accounts' : 'Manage your school efficiently'
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="ml-1">
            <h1 className="text-[15px] font-bold text-gray-900 tracking-tight">{dashboardTitle}</h1>
            <p className="text-xs text-gray-400 hidden md:block">{dashboardSub}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div ref={notifRef} className="relative">
            ...
            {isNotifOpen && (
              <NotifDropdown onClose={() => setIsNotifOpen(false)} basePath={basePath} />
            )}
          </div>
            ...
            >
              ...
              <div className="hidden md:block text-left">
                <p className="text-[13px] font-semibold text-gray-900 leading-tight">
                  {user?.name || (isAccountant ? 'Accountant' : 'Administrator')}
                </p>
                <p className="text-[11px] text-gray-400 capitalize leading-tight">
                  {isAccountant ? 'Accountant' : (user?.role?.replace('_', ' ') || 'School Admin')}
                </p>
              </div>
            ...
            {isUserMenuOpen && (
              ...
              >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className={`text-[11px] font-semibold capitalize ${
                          isAccountant ? 'text-emerald-600' : 'text-blue-600'
                        }`}>
                          {isAccountant ? 'Accountant' : (user?.role?.replace('_', ' ') || 'School Admin')}
                        </span>
                      </div>
                ...
                <div className="p-2 space-y-0.5">
                  {/* My Profile */}
                  <button
                    onClick={() => { setIsUserMenuOpen(false); navigate(`${basePath}/profile`) }}
                  ...
                  {/* My Notifications */}
                  <button
                    onClick={() => { setIsUserMenuOpen(false); navigate(`${basePath}/my-notifications`) }}
                    className="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-xl transition-colors group"
                  >
